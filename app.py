...
def get_wagon_fill_html(contents, capacity):
    ...
    for i in range(capacity):
        html += cfg.GOODS_HTML.get(
            contents[i]["good"] if i < len(contents) else None, cfg.GOODS_HTML[None]
        )
    return html
...
def initialize_app():
    ...
    settings = {
        ...
        "REPAIR_LOCO": cfg.REPAIR_LOCO,
        "REPAIR_WAGON": cfg.REPAIR_WAGON,
        "GOODS_PRICES": cfg.GOODS_PRICES.copy(),
        "WAGON_PRICES": cfg.WAGON_PRICES.copy(),
        "WAGON_INFO": cfg.WAGON_INFO,
...
tab_game, tab_instruction, tab_settings = st.tabs(
    ["🕹️ Игра", "📖 Инструкция", "⚙️ Настройки"]
)
...
with tab_game:
    ...
    with col1:  # КОЛОНКА ПОЕЗДА И РЕМОНТА
        ...
        for i in range(1, 6):
            ...
            if state[f"wagon_{i}_is_purchased"]:
                ...
                fill_html = (
                    get_platform_fill_html(state[f"wagon_{i}_contents"], capacity)
                    if wagon_info["type"] == "platform"
                    else get_wagon_fill_html(state[f"wagon_{i}_contents"], capacity)
                )
                ...
                row_cols[0].markdown(
                    f"<span style='color:grey;'>{wagon_info['name']}</span>",
                    unsafe_allow_html=True,
                )
                row_cols[1].markdown(
                    "<span style='color:grey;'>-</span>", unsafe_allow_html=True
                )
                row_cols[2].markdown(
                    "<span style='color:grey;'>Не куплен</span>", unsafe_allow_html=True
                )
                if state["moves_made_this_round"] == 0 and state["station"] == "A":
                    ...
                    if row_cols[3].button(f"Купить ({price}₽)", key=f"buy_wagon_{i}"):
                        if state["money"] >= price:
                            st.session_state.game_state = ge.perform_action(
                                state, "buy_wagon", wagon_index=i
                            )
                            st.rerun()
                        else:
        ...
        st.markdown("")
        if (
            state["moves_made_this_round"] == 0
            and state["station"] == "A"
            and not state["repaired_this_round"]
        ):
            with st.container(border=True):
                ...
                if 0 < state["loco_hp"] < 3:
                    needs_repair = True
                    cost = int(
                        state["settings"]["REPAIR_LOCO"]
                        * state["modifiers"]["repair_cost_multiplier"]
                    )
                    cols = st.columns(2)
                    cols[0].markdown("**Локомотив**")
                    if cols[1].button(
                        f"Ремонт ({cost}₽)",
                        key="depot_repair_loco",
                ...
                for i in range(1, 6):
                    if (
                        ...
                        and 0 < state[f"wagon_{i}_hp"] < 3
                    ):
                        needs_repair = True
                        cost = int(
                            state["settings"]["REPAIR_WAGON"]
                            * state["modifiers"]["repair_cost_multiplier"]
                        )
                        cols = st.columns(2)
                        cols[0].markdown(
                            f"**{state['settings']['WAGON_INFO'][i]['name']}**"
                        )
                        if cols[1].button(
                            f"Ремонт ({cost}₽)",
                            key=f"depot_repair_wagon_{i}",
                            use_container_width=True,
                        ):
                            if state["money"] >= cost:
                                st.session_state.game_state = ge.perform_action(
                                    state, "repair_wagon", wagon_index=i
                                )
                                st.rerun()
                            else:
                                st.error("Недостаточно денег!")
    ...
    with col2:  # КОЛОНКА КОНТРАКТОВ
        ...
        if not state["active_contracts"]:
            ...
            for contract in state["active_contracts"]:
                ...
                if contract["goods_2"]:
                    goods_str += f", {contract['goods_2']}×{contract['qty_2']}"
                if contract["goods_3"]:
                    goods_str += f", {contract['goods_3']}×{contract['qty_3']}"
                ...
                act_cols[2].markdown(
                    f"{goods_str} ({ge.calculate_current_price(contract, state)}₽)"
                )
                act_cols[3].markdown(f"{contract['rounds_left']}")
                with act_cols[4]:
                    loading_forbidden = (
                        state["station"] == "A" and state["moves_made_this_round"] >= 2
                    )
                    if (
                        not contract.get("is_loaded")
                        and state["station"] == contract["origin"]
                        and not loading_forbidden
                    ):
                        if st.button("Погрузка", key=f"load_{contract['id']}"):
                            if ge.check_capacity_for_contract(state, contract):
                                st.session_state.game_state = ge.perform_action(
                                    state, "load_contract", contract_id=contract["id"]
                                )
                                st.rerun()
                            else:
                                st.error("Недостаточно места!")
                    if (
                        contract.get("is_loaded")
                        and state["station"] == contract["destination"]
                    ):
                        if st.button("Разгрузка", key=f"unload_{contract['id']}"):
                            st.session_state.game_state = ge.perform_action(
                                state, "unload_contract", contract_id=contract["id"]
                            )
                            st.rerun()
        st.markdown("")
        if state["moves_made_this_round"] == 0:
            with st.container(border=True):
                ...
                if len(state["active_contracts"]) >= 4:
                    ...
                    s_pool = [
                        c for c in state["contracts_pool"] if c["id"].startswith("P")
                    ]
                    m_pool = [
                        c for c in state["contracts_pool"] if c["id"].startswith("M")
                    ]
                    h_pool = [
                        c for c in state["contracts_pool"] if c["id"].startswith("S")
                    ]
                    if contract_cols[0].button(
                        f"Простой ({len(s_pool)})",
                        disabled=(not s_pool or not can_take),
                        use_container_width=True,
                    ):
                        st.session_state.game_state = ge.perform_action(
                            state, "take_contract", ctype="P"
                        )
                        st.rerun()
                    if contract_cols[1].button(
                        f"Средний ({len(m_pool)})",
                        disabled=(not m_pool or not can_take),
                        use_container_width=True,
                    ):
                        st.session_state.game_state = ge.perform_action(
                            state, "take_contract", ctype="M"
                        )
                        st.rerun()
                    if contract_cols[2].button(
                        f"Сложный ({len(h_pool)})",
                        disabled=(not h_pool or not can_take),
                        use_container_width=True,
                    ):
                        st.session_state.game_state = ge.perform_action(
                            state, "take_contract", ctype="S"
                        )
                        st.rerun()
    ...
    with col3:  # КОЛОНКА ДЕЙСТВИЙ
        with st.container(border=True):
            ...
            if st.button(
                "Двигаться",
                disabled=(state["moves_made_this_round"] >= 2),
                use_container_width=True,
            ):
            ...
            st.markdown("---")
            if st.button(
                "Начать новую игру", type="secondary", use_container_width=True
            ):
...
with tab_instruction:
    ...
    **Этап 3: Возвращение и конец раунда**
    ...
    st.subheader(
        "🛠️ Секреты успешного машиниста (Важные правила!)",
    )

    st.markdown("**1. Меньше сердечек — меньше места в вагоне!**")
    st.markdown(
        "Поврежденный вагон не может везти столько же груза, сколько новый. Это самое важное правило!"
    )
    st.table(
        {
    ...
    st.table(
        {
            ...
            "Опоздание на 1 раунд": [
                "Получишь 90% цены",
                "Получишь 70% цены",
                "Получишь 40% цены",
            ],
            "Опоздание на 2+ раунда": [
                "Получишь всего 20-60% цены",
                "Получишь всего 30% цены",
    ...
    st.markdown(
        "Ты не можешь положить уголь в вагон для ящиков! Это как пытаться налить суп в карман."
    )
    st.markdown(
        """
...
with tab_settings:
    ...
    with st.expander("Основные экономические параметры", expanded=True):
        st.session_state.game_settings["STARTING_MONEY"] = st.number_input(
            "Стартовые деньги (₽)",
            0,
            step=500,
            value=st.session_state.game_settings["STARTING_MONEY"],
        )
        st.session_state.game_settings["REPAIR_LOCO"] = st.number_input(
            "Стоимость ремонта локомотива (₽)",
            0,
            step=50,
            value=st.session_state.game_settings["REPAIR_LOCO"],
        )
        st.session_state.game_settings["REPAIR_WAGON"] = st.number_input(
            "Стоимость ремонта вагона (₽)",
            0,
            step=50,
            value=st.session_state.game_settings["REPAIR_WAGON"],
        )
    st.subheader("Цены на покупку вагонов")
    cols = st.columns(2)
    for i in range(1, 6):
        wagon_name = cfg.WAGON_INFO[i]["name"]
        st.session_state.game_settings["WAGON_PRICES"][i] = cols[
            (i - 1) % 2
        ].number_input(
            f"Цена '{wagon_name}'",
            0,
            step=100,
            value=st.session_state.game_settings["WAGON_PRICES"][i],
        )
    ...
    for i, good in enumerate(good_keys):
        st.session_state.game_settings["GOODS_PRICES"][good] = cols[i % 3].number_input(
            f"Цена '{good}'",
            0,
            step=10,
            value=st.session_state.game_settings["GOODS_PRICES"][good],
        )
    st.markdown("---")
