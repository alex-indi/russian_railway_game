...
def check_capacity_for_contract(state, contract):
    ...
    available_space = {
        i: get_current_capacity(state, i) - len(state[f"wagon_{i}_contents"])
        for i in range(1, 6)
        if state[f"wagon_{i}_is_purchased"]
    }
    gondola_type = {
        i: state[f"wagon_{i}_contents"][0]["good"]
        for i in available_space
        if state["settings"]["WAGON_INFO"][i]["type"] == "gondola"
        and state[f"wagon_{i}_contents"]
    }
    for good in goods_to_load:
        ...
        for i in sorted(available_space.keys()):
            if (
                state["settings"]["WAGON_INFO"][i]["type"] == compatible_wagon_type
                and available_space[i] > 0
            ):
...
def apply_event_effect(state, event):
    ...
    if event_id in ["E01", "E11"]:
        if state["loco_hp"] > 0:
            state["loco_hp"] -= 1
            if state["loco_hp"] <= 0:
                state["game_over"], state["game_over_reason"] = (
                    True,
                    "Локомотив сломан!",
                )
        ...
        for i in range(1, 6):
            if state[f"wagon_{i}_is_purchased"]:
                state[f"wagon_{i}_hp"] = min(3, state[f"wagon_{i}_hp"] + 1)
        ...
        if random.randint(1, 6) == 1 and state["loco_hp"] > 0:
            ...
            if state["loco_hp"] <= 0:
                state["game_over"], state["game_over_reason"] = (
                    True,
                    "Локомотив сломан в снегопад!",
                )
        for i in range(1, 6):
            if (
                state[f"wagon_{i}_is_purchased"]
                and state[f"wagon_{i}_hp"] > 0
                and random.randint(1, 6) == 1
            ):
                state[f"wagon_{i}_hp"] -= 1
                if state[f"wagon_{i}_hp"] <= 0:
                    state[f"wagon_{i}_is_purchased"], state[f"wagon_{i}_contents"] = (
                        False,
                        [],
                    )
    ...
    if state["money"] < 0:
        state["game_over"], state["game_over_reason"] = (
            True,
            f"Банкротство из-за события «{event['name']}»!",
        )
    ...
    for c in state["active_contracts"]:
        if c["id"] == contract_id:
            ...
            for g, q in [
                ...
                if g and q:
                    goods_to_load.extend([{"good": g, "contract_id": c["id"]}] * q)
            for item in goods_to_load:
                ctype = state["settings"]["GOOD_COMPATIBILITY"][item["good"]]
                for i in range(1, 6):
                    if (
                        state[f"wagon_{i}_is_purchased"]
                        and state["settings"]["WAGON_INFO"][i]["type"] == ctype
                        and len(state[f"wagon_{i}_contents"])
                        < get_current_capacity(state, i)
                    ):
                        if (
                            ctype == "gondola"
                            and state[f"wagon_{i}_contents"]
                            and state[f"wagon_{i}_contents"][0]["good"] != item["good"]
                        ):
                            continue
...
def initialize_state(settings):
    state = {
        ...
        "station": "A",
        "loco_hp": 3,
        "contracts_pool": [c.copy() for c in CONTRACTS],
        "active_contracts": [],
        "completed_contracts": [],
...
def perform_action(state, action, **kwargs):
    ...
    if (
        action == "move"
        and new_state["moves_made_this_round"] < 2
        and new_state["time"] >= time_cost_move
    ):
        ...
        price = 0
        contract_to_remove = next(
            (c for c in new_state["active_contracts"] if c["id"] == contract_id), None
        )
        if contract_to_remove:
            for i in range(1, 6):
                if new_state[f"wagon_{i}_is_purchased"]:
                    new_state[f"wagon_{i}_contents"] = [
                        item
                        for item in new_state[f"wagon_{i}_contents"]
                        if item["contract_id"] != contract_id
                    ]
            price = calculate_current_price(
                contract_to_remove, new_state
            )  # Передаем new_state
        ...
        if action == "repair_loco":
            cost = int(
                new_state["settings"]["REPAIR_LOCO"]
                * new_state["modifiers"]["repair_cost_multiplier"]
            )
            ...
            wagon_index = kwargs["wagon_index"]
            cost = int(
                new_state["settings"]["REPAIR_WAGON"]
                * new_state["modifiers"]["repair_cost_multiplier"]
            )
            new_state["money"] -= cost
            new_state[f"wagon_{wagon_index}_hp"] = 3
        ...
        ctype = kwargs["ctype"]
        contract_pool = [
            c for c in new_state["contracts_pool"] if c["id"].startswith(ctype)
        ]
        if contract_pool and len(new_state["active_contracts"]) < 4:
            ...
            chosen_contract["is_loaded"], chosen_contract["rounds_left"] = (
                False,
                chosen_contract["max_rounds"],
            )
            new_state["active_contracts"].append(chosen_contract)
            new_state["contracts_pool"] = [
                c
                for c in new_state["contracts_pool"]
                if c["id"] != chosen_contract_orig["id"]
            ]
        ...
        if random.randint(1, 6) == 1 and new_state["loco_hp"] > 0:
            ...
            if new_state["loco_hp"] <= 0:
                new_state["game_over"], new_state["game_over_reason"] = (
                    True,
                    "Износ локомотива.",
                )
                return new_state
        for i in range(1, 6):
            if (
                new_state[f"wagon_{i}_is_purchased"]
                and new_state[f"wagon_{i}_hp"] > 0
                and random.randint(1, 6) == 1
            ):
